...
def heuristic_score_readme(
    ...
    # Truncate to match ScoreResult.summary's max_length=800. The heuristic
    # doesn't have an LLM to write a full project description, so it falls
    # back to the repo's GitHub description (up to 350 chars) plus a stub
    # if that's empty.
    summary = (repo.description.strip() or "README 中未提供明确描述")[:800]
    reason = (
